...
const buf = readFileSync('/sessions/bold-friendly-fermi/mnt/uploads/Tableau_commandes_CHABANEL.xlsx');
...
console.log(JSON.stringify({ totalFormulas: validation.totalFormulas, evaluated: validation.evaluated, matches: validation.matches, parseErrors: validation.parseErrors }, null, 2));
...
console.log('--- recettes ---', model.recettes.map(r => r.pate + ' <- ' + r.feuille + ' (' + r.lignes.length + ' lignes, base ' + r.base + ')'));
