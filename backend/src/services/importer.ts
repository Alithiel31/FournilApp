...
import { arrondiFor, type ImportModel, type ValidationResult } from '../import/extract';

export async function applyImport(model: ImportModel, fileName: string, validation: ValidationResult) {
  return prisma.$transaction(async (tx) => {
    ...
    const nomsPates = [...new Set(model.produits.map((p) => p.pate).filter((x): x is string => !!x))];
